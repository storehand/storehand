...
### One that is specific to this project

Publishing someone else's store data — a `*.myshopify.com` handle, a shop id, an
order, a customer's name, email or address — counts as publishing private
information, whether it arrives in an issue, a pull request, or a screenshot. It
is the one mistake here that cannot be undone by a follow-up commit. If it
happens by accident, say so immediately so it can be removed; nobody is in
trouble for reporting their own slip.
...
[private security advisory](https://github.com/storehand/storehand/security/advisories/new),
which is the only private channel this project has. If you do not have access to
that, open an issue saying only that you have a report and asking for a contact
— no details in the issue itself. This mirrors how security reports are handled;
